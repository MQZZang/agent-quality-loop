...
| 3.2.1 information-alignment consistency patch | SPEC + STATIC + FORCED_LOAD AGENT_REVIEW | Source-level conflicts and focused regression coverage are implemented. On `gpt-5.6-terra` / Codex / medium, retained forced-load A/B was parity: each arm 13 PASS / 3 NOT_DECIDABLE / 0 hard-boundary violations across the same 16 synthetic cases. A broader follow-up was rejected for a matched improvement/regression. The two initially held-out cases lost that status after round-1 tuning. Natural discovery, native Excel/WorkBuddy behavior, cross-model generalization, and product benefit remain `NOT_RUN`. Evidence: `docs/experiments/aql-3.2-information-alignment/`. |
