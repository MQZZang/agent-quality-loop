...
AQL 3.2 只有一个 Skill：`agent-quality-loop`。你照常用自然语言说明要做什么，它会把结果、范围、证据和权限整理进同一份 Task Contract。独立验收通过，也不代表可以发布。

当前发布 tag 是 `v3.1.1`，分支顶端是 `3.2.0` 工作源码。需要不可变的已发布产物时请检出 tag；使用当前工作源码时用
分支即可。不要用 `v3.1.0`：它自带的自测在 2026-08-18 之后的任何日期都会失败，已在 3.1.1
修复。GitHub Release 只有在精确 tag 通过发布工作流后才存在。CI 当前使用 Node.js 22，建议保持一致。
