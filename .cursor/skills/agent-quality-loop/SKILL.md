...
**Grounding:** Compile from observed reality, not only from the user's words — users often cannot name the real need, so the request's description of the system is itself a hypothesis. Before freezing the contract, verify its load-bearing referents (named files, behaviors, terms) read-only against the current environment per the [grounding ladder](references/contracts.md#grounding-ladder); when the compile depends on facts or conventions outside the project, verify against an authoritative source rather than model memory and record source and date. A premise contradicted by observation is a contradiction — disclose and resolve it toward the outcome **before any edit**; never fabricate the missing referent to satisfy the letter of the request. A requested mechanism compiles as the outcome it serves plus an `assumptions` entry, never as the goal itself. Grounding depth follows the ceremony budget: `fast` checks only referents the task already touches and never turns a trivial task into a research project.
...
| Compiles the goal only from the user's words when one read would falsify the premise | Ground load-bearing referents before the freeze; disclose the mismatch |
